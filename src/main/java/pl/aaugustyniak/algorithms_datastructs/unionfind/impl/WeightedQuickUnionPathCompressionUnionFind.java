 ...
 * @param <T>
 */
public class WeightedQuickUnionPathCompressionUnionFind<T> implements UnionFind<T> , ArrayAccessCostModel{
   ...
   List<T> objects = new ArrayList<>();
    ...
    private int findRoot(T p) {
        int root = this.objects.indexOf(p);
        while (root != this.id.get(root)) {
            cost++;
            this.id.set(root , this.id.get(this.id.get(root)));
            root = this.id.get(root);
